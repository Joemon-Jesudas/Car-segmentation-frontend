...
def preprocessImage(img):
    ...
    if im.size[0] > im.size[1]:
        ...
        im = im.resize((im_width, hsize), Image.LANCZOS)
    else:

        hpercent = (im_height / float(im.size[1]))
        wsize = int((float(im.size[0]) * float(hpercent)))
        im = im.resize((wsize, im_height), Image.LANCZOS)
...
def predict_image(image):
    ...
    model = load_model('my_model2.hdf5', custom_objects={'dice_coef':dice_coef, 'dice_coef_loss':dice_coef_loss},compile=False)
